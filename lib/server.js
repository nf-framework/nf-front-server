...
function replacer(options) {
    return (m, s, f, e) => {
        ...
        const route = match && api.getRouteMap(match[1]);
        ...
        if (route && route.options && route.options.replace) {
            ...
            try {
                if (match && match[1] && match[2]) {
                    try {
                        const pj = JSON.parse(fs.readFileSync(process.cwd() + '/node_modules/' + f + '/package.json', 'utf8'));
                        const file = /* (options && options.cacheKeyExtension == 0) && */ (pj.module || pj['jsnext:main']) || pj.main || match[1];
                        f += '/' + file;
                    } catch { /* empty */ }
                }
                if (match && match[1] && !match[2]) {
                    // указано только имя библиотеки, выясняем что нужно загружать
                    const pj = JSON.parse(fs.readFileSync(process.cwd() + '/node_modules/' + match[1] + '/package.json', 'utf8'));
                    const file = /* (options && options.cacheKeyExtension == 0) && */ (pj.module || pj['jsnext:main']) || pj.main || match[1];
                    f += '/' + file;
                }
                match = f.match(/(\/nf-.*|\.[^.\/]+)$/);
...
function registerLibDir(lib, path, options) {
    ...
    web.on('GET', `/${lib}${options?.singleFile ? '' : '/*'}`, options || {}, async (context) => {
        ...
        const req_path = sanitizePath('/' + context.store.lib, userPath);
    ...
    if (options?.recursive) {
        ...
        for (const el of dirs) {
            ...
            if (dirStat.isDirectory()) {
                await registerCustomElementsDir(lib + '/' + el);
            } else {
