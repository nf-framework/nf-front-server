...
function replacer(options) {
    return (m, s, f, e) => {
        let match = f.match(/^\/?([@\w-]*)(\/.*)?/);
        let route = api.getRouteMap(match[1]);
...
async function prepareResponse(cacheKey, options, cb) {
    ...
    if (moduleConfig.modules.serverCache) {
        if (!cache.has(cacheKey)) {
            ...
            if (['text/html', 'application/javascript', 'text/css', 'image/svg+xml'].includes(options.mimeType)) {
        ...
        const c = await cb();
        if (c) {
            const headers = { 'Content-Type': options.mimeType+'; charset=UTF-8' };
...
function registerLibDir(lib, path, options) {
    ...
    web.on('GET', `/${lib}${options?.singleFile ? '' : '/*'}`, {}, async context => {
...
async function cmpInitHandler(context) {
    ...
    const response = await prepareResponse(cacheKey,
        { customOptions, contentType: 'javascript', mimeType: 'text/javascript' },
        async () => {
...
export { replacer, getCacheKey, prepareResponse, registerLibDir }