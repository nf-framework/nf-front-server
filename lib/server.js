...
async function registerCustomElementsDir(lib, path, options) {
    ...
    if (options?.recursive) {
        let dirs =  await fsProm.readdir(path);
        for (let dir of dirs) {
            await registerCustomElementsDir(lib + '/' + dir);

        }
    } else {
        try {
            let filesNames = await fsProm.readdir(path);
            let files = filesNames.filter(x => x.endsWith('.js'));

            for (let el of files) {
                let req_path = `${lib}/${el}`;
                let el_path = null
                if (options?.overrideLib?.[el]) {
                    req_path = options.overrideLib[el];
                ...
                let themeStylePath = await extension.getFiles(`/themes/${config.theme}/${el.replace('.js', '.css')}`);
