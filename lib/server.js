...
if (!buildHash || buildHash === '') {
    ...
    console.log('URL cache string: \'%s\'', buildHash);
}
...
/**
 * Search and register customElements from directory.
 * @param {string } lib - Library/Component full name
 * @param {string?} path - path to directory if different from standard
 * @param options
 * @param {boolean} options.recursive - Recursive directory search
 * @return {Promise<void>}
 */
async function registerCustomElementsDir(lib, path, options) {
    ...
    if (options?.recursive) {
            ...
            if (dirStat.isDirectory()) {
                ...
                if (el.endsWith('.js')) {
                    if (el.indexOf('-') > 0) {
                        ...
                        // Mark to include directory as lib-dir
                        hasJSInDir = true;
                    }
                }
            }
        ...
        if (hasJSInDir) {
            registerLibDir(lib, path, options);
        }
    } else {
        try {
...
async function cmpInitHandler(context) {
    ...
    const response = await prepareResponse(cacheKey,
        ...
        async () => {
            ...
            if (moduleConfig.modules.clientURLCacheString === 'simple') {
                streams.push(Stream.Readable.from(['\nwindow.NFURLCacheString="', buildHash, '"; //12']));
            }
